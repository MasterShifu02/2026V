...
export function BlackHoleEscapeRoom({
  ...
  const handleMusicToggle = () => {
    if (!musicAudioRef.current) {
      setMusicNotice("Fant ikke startsangen. Legg inn StartTheme.mp3.");
      return;
    }
  ...
  return (
    <main className="black-hole-page">
      ...
      <section className="escape-card">
        <div className="escape-music-row">
          <button type="button" className="escape-music-button" onClick={handleMusicToggle}>
            {isMusicPlaying ? "Pause music" : "Play music"}
          </button>
          {musicNotice && <p className="escape-music-status">{musicNotice}</p>}
        </div>
        {showDevSkip && (
